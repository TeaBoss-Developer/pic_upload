...
function qy(){
    flag=true
    alert(prompt("请输入要迁移到的IP"))
}
let isDragging = 0;
window.onload = function (){
    
    ajax('http://tea.fly09.com:8886/list',"",function(data){
        data = JSON.parse(data);
        for(let i in data['data']){
            if(data['data'][i][Object.keys(data['data'][i])[0]]!=0){
                file={
                    name:Object.keys(data['data'][i])[0].split('/')[4].toUpperCase(),
                    size:data['data'][i][Object.keys(data['data'][i])[0]],
                }
                if(Object.keys(data['data'][i])[0].indexOf("jpg")==-1 && Object.keys(data['data'][i])[0].indexOf("jpeg")==-1 && Object.keys(data['data'][i])[0].indexOf("png")==-1 && Object.keys(data['data'][i])[0].indexOf("bmp")==-1 && Object.keys(data['data'][i])[0].indexOf("gif")==-1 ){
                    itemMarkupOffline(file,Object.keys(data['data'][i])[0],50,50,"/R-C.png")
                }else{
                    itemMarkupOffline(file,Object.keys(data['data'][i])[0],50,50,Object.keys(data['data'][i])[0])
                }
            }
        }
    })
...
document.addEventListener("dragover", e => {
    ...
    isDragging++;
    if (isDragging === 1)
        droppable.classList.add("is-dragging")
}
);
...
const itemMarkup = (file, url, x, y) => {
    console.warn(file,url,x,y)
    const item = document.createElement("div");
    const id = Math.random().toString(36).substr(2, 9);
    item.classList.add("item");
    item.setAttribute("id", id);
    item.innerHTML = `<div class="item-img"><a href=${url}><img src=${url}></a></div><div class="item-details"><div class="item-name">${file.name}</div><div class="item-size">SIZE:${formatBytes(file.size)}</div></div><button class="item-delete"data-id="${id}"></button><button class="item-delete item-url"id="${id}iAjue"onclick="copyToClipboard(this)">复制</button>`;
    list.append(item);
    ...
    ajax('http://tea.fly09.com:8886/upload', file, function (data) {
        data = JSON.parse(data);
        if (data.code == 0) {
            const itemurlBtn = item.querySelector(".item-url");
            itemurlBtn.setAttribute('data-url', location.origin + data.url);
    ...
    ;
    const itemMarkupOffline = (file, url, x, y,imageurl) => {
        const item = document.createElement("div");
        const id = Math.random().toString(36).substr(2, 9);
        item.classList.add("item");
        item.setAttribute("id", id);
        item.innerHTML = `<div class="item-img"><a href=${url}><img style="object-fit: file;" src=${imageurl}></a></div><div class="item-details"><div class="item-name">${file.name}</div><div class="item-size">SIZE:${formatBytes(file.size)}</div></div><button class="item-delete"data-id="${id}"></button><button class="item-delete item-url"id="${id}iAjue"onclick="copyToClipboard(this)">复制</button>`;
        list.append(item);
        const itemDeleteBtn = item.querySelector(".item-delete");
        itemDeleteBtn.addEventListener("click", e => {
            e.stopPropagation();
        ...
        const loadedSVG = image.querySelector("span");
        const itemurlBtn = item.querySelector(".item-url");
            itemurlBtn.setAttribute('data-url', url);
            itemurlBtn.addEventListener('click', function (e) {
                e.stopPropagation()
            }, false);
        tl.set(droppable, {
...
const deleteItem = e => {
    ...
    ajax('http://tea.fly09.com:8886/delete',document.getElementById(parent.children[3].id).getAttribute('data-url'),function (data){
        console.debug(document.getElementById(parent.children[3].id).getAttribute('data-url')+"删除成功(已在云端删除)")
    });
